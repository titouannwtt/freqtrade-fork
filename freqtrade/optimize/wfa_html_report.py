...
<th>Calmar</th><th>Max DD</th><th>WFE</th><th>Regime</th></tr>
{rows}
</table>
</div>"""
...
{self._svg_equity_curve(profits)}
    ...
    def _section_monte_carlo(self) -> str:
        ...
        return f"""
<div class="section">
<h2>Monte Carlo Shuffle ({mc["n_simulations"]} sims)</h2>
<div>
  <span class="kv"><span class="kv-label">Return</span><br>
    <span class="kv-value">{mc["total_return_pct"]:+.1f}%</span></span>
  <span class="kv"><span class="kv-label">Max DD p5/p50/p95</span><br>
...
{mc["return_dd_p95"]:.2f}</span></span>
  <span class="kv"><span class="kv-label">Carver discount</span><br>
    <span class="kv-value">{mc["carver_discount"]:.2f}</span></span>
  <span class="kv"><span class="kv-label">Consec loss p95</span><br>
    ...
    def _section_phase3(self) -> str:
        ...
        if perturb:
            parts.append(
                f"<h3>Parameter Perturbation ({perturb['n_perturbations']} variants)</h3>"
                f"<div>"
                f"<span class='kv'><span class='kv-label'>Profit p5/p50/p95</span><br>"
                f"<span class='kv-value'>{perturb['profit_p5']:+.1f}% / "
                f"{perturb['profit_p50']:+.1f}% / "
                f"{perturb['profit_p95']:+.1f}%</span></span>"
                f"<span class='kv'><span class='kv-label'>% Profitable</span><br>"
                f"<span class='kv-value'>{perturb['pct_profitable']:.0%}</span></span>"
                f"<span class='kv'><span class='kv-label'>Sensitivity</span><br>"
                f"<span class='kv-value'>{perturb['sensitivity']:.2f}</span></span>"
                f"</div>"
            )
        ...
        if ms:
            parts.append(
                ...
                f"<p>Convergence: {ms['convergence_pct']:.0%}</p>"
            )

        if not parts:
            return ""
        return '<div class="section"><h2>Robustness</h2>' + "\n".join(parts) + "</div>"
    ...
    def _section_cpcv(self) -> str:
        ...
        return f"""
<div class="section">
<h2>CPCV (N={cpcv["n_groups"]}, K={cpcv["n_test_groups"]})</h2>
<div>
  ...
  <span class="kv"><span class="kv-label">Sharpe of Paths</span><br>
    <span class="kv-value">{cpcv["sharpe_of_paths"]:.2f}</span></span>
  <span class="kv"><span class="kv-label">P(loss)</span><br>
    <span class="kv-value">{cpcv["prob_of_loss"]:.0%}</span></span>
